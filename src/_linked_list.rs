...
impl quote::ToTokens for LinkedListComprehension {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let mut nested_code = match &self.mapping.right_expr {
            None => quote! {
                __rusthension_linked_list.push_back(#left_expr);
            },
            Some(MappingElse {
                conditions,
                else_expr,
                ...
                quote! {
                    if #conditions {
                        ...
                        __rusthension_linked_list.push_back(#else_expr);
                    }
                }
            }
        };
...
mod tests {
    ...
    fn test_comprehension_parse() {
        // 测试基本的列表推导式解析
        let comprehension: LinkedListComprehension = parse_quote! {
        ...
        let comprehension: LinkedListComprehension = parse_quote! { [x, y]
            if x > y else (y, x)
